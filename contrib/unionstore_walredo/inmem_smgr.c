...
smgr_hook_type inmen_prev_smgr_hook = NULL;
...
const struct f_smgr inmem_smgr =
{
...
void
inmem_smgr_init(SMgrRelation reln, BackendId backend, SMgrImpl which, Relation rel)
{
	bool match = false;

	/*
	 * set inmem storage manager here
	 */
	if (which == SMGR_MD && backend == InvalidBackendId)
	{
		if (am_wal_redo_postgres)
		{
			reln->smgr = &inmem_smgr;
	...
	/*
	 * not inmen, keep going
	 */
	if (!match && inmen_prev_smgr_hook)
	    (*inmen_prev_smgr_hook)(reln, backend, which, rel);
}
