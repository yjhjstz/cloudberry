 ...
 */
	if (strstr(url, LocalhostIpV4) != NULL)
	{
		...
		char	   *dlproxy_host_entry = (char *) palloc0(LOCAL_HOST_RESOLVE_STRING_MAX_LENGTH);

		snprintf(dlproxy_host_entry, LOCAL_HOST_RESOLVE_STRING_MAX_LENGTH, LOCAL_HOST_RESOLVE_STRING_FORMAT, get_dlproxy_port());
		elog(DEBUG3, "adding CURLOPT_RESOLVE with entry '%s'", dlproxy_host_entry);
		resolve_hosts = curl_slist_append(NULL, dlproxy_host_entry);
		set_curl_option(context, CURLOPT_RESOLVE, resolve_hosts);
		pfree(dlproxy_host_entry);
	}
...
get_dest_address(CURL *curl_handle)
{
	...
	{
		return psprintf("'%s:%d'", dest_url, get_dlproxy_port());
	}
	return dest_url;
}
		   ...
		   ((context->download_buffer->top - context->download_buffer->bot) < want))
	{
		...
		else if (-1 == select(maxfd + 1, &fdread, &fdwrite, &fdexcep, &timeout))
		{
			...
			elog(ERROR, "internal error: select failed on curl_multi_fdset (maxfd %d) (%d - %s)",
				 maxfd, errnonum, strerror(errnonum));
		}
		multi_perform(context);
	}
}
...
check_response_code(churl_context *context)
{
	...
	else if (response_code != 200 && response_code != 100)
	{
		...
		appendStringInfo(&err, "dlproxy server error");
...
get_http_error_msg(long http_ret_code, char *msg, char *curl_error_buffer, char **hint_message, char **trace_message)
{
	...
	if (http_ret_code == 0)
	{
		*hint_message = "Use 'gpstop -ar' command to restart the dlproxy service.";
		if (curl_error_buffer == NULL)
			return "There is no dlproxy servlet listening on the host and port specified in the external table url";
		else
		{
...
handle_special_error(long response, StringInfo err)
{
	...
	{
		case 404:
			appendStringInfo(err, ": dlproxy service could not be reached. dlproxy is not running in the tomcat container");
			break;
		default:
			return false;
	}
