...
CREATE WRITABLE EXTERNAL TABLE one_file(a int, b text) LOCATION('gphdfs://ci-test-data/orc/onefile hdfs_cluster_name=paa_cluster') FORMAT 'orc';
insert into one_file values(1, 'aaaaabbbb');

DROP EXTERNAL TABLE IF EXISTS one_file2;
CREATE WRITABLE EXTERNAL TABLE one_file2(a int, b text) LOCATION('gphdfs://ci-test-data/orc/onefile2 hdfs_cluster_name=paa_cluster') FORMAT 'orc';
insert into one_file2 values(1, 'aaaaabbbb');

DROP EXTERNAL TABLE IF EXISTS two_file;
CREATE WRITABLE EXTERNAL TABLE two_file(a int, b text) LOCATION('gphdfs://ci-test-data/orc/twofile hdfs_cluster_name=paa_cluster') FORMAT 'orc';
insert into two_file values(1, 'aaaaabbbb');
insert into two_file values(1, 'aaaaabbbb');

DROP EXTERNAL TABLE IF EXISTS more_file;
CREATE WRITABLE EXTERNAL TABLE more_file(a int, b text) LOCATION('gphdfs://ci-test-data/orc/more_file hdfs_cluster_name=paa_cluster') FORMAT 'orc';
...
CREATE WRITABLE EXTERNAL TABLE more_file2(a int, b text) LOCATION('gphdfs://ci-test-data/orc/more_file2 hdfs_cluster_name=paa_cluster') FORMAT 'orc';
