...
CREATE WRITABLE EXTERNAL TABLE one_file(a int, b text) LOCATION('gphdfs://ci-test-data/parquet/onefile hdfs_cluster_name=paa_cluster') FORMAT 'parquet';
insert into one_file values(1, 'aaaaabbbb');

DROP EXTERNAL TABLE IF EXISTS one_file2;
CREATE WRITABLE EXTERNAL TABLE one_file2(a int, b text) LOCATION('gphdfs://ci-test-data/parquet/onefile2 hdfs_cluster_name=paa_cluster') FORMAT 'parquet';
insert into one_file2 values(1, 'aaaaabbbb');

DROP EXTERNAL TABLE IF EXISTS two_file;
CREATE WRITABLE EXTERNAL TABLE two_file(a int, b text) LOCATION('gphdfs://ci-test-data/parquet/twofile hdfs_cluster_name=paa_cluster') FORMAT 'parquet';
insert into two_file values(1, 'aaaaabbbb');
insert into two_file values(1, 'aaaaabbbb');

DROP EXTERNAL TABLE IF EXISTS more_file;
CREATE WRITABLE EXTERNAL TABLE more_file(a int, b text) LOCATION('gphdfs://ci-test-data/parquet/more_file hdfs_cluster_name=paa_cluster') FORMAT 'parquet';
...
CREATE WRITABLE EXTERNAL TABLE more_file2(a int, b text) LOCATION('gphdfs://ci-test-data/parquet/more_file2 hdfs_cluster_name=paa_cluster') FORMAT 'parquet';
