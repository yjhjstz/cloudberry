...
#define UNION_STORE_AM "union_store"

static Oid  UnionStoreAmId = InvalidOid;

static Oid  US_HASH_AM_OID = InvalidOid;
static Oid  US_BTREE_AM_OID = InvalidOid;
static Oid  US_GIST_AM_OID = InvalidOid;
static Oid  US_GIN_AM_OID = InvalidOid;
static Oid  US_SPGIST_AM_OID = InvalidOid;
static Oid  US_BRIN_AM_OID = InvalidOid;
static Oid  US_BITMAP_AM_OID = InvalidOid;
...
/* previous hook for smgr */
smgr_hook_type neon_prev_smgr_hook = NULL;

ProcessUtility_hook_type neon_prev_ProcessUtility = NULL;

/*
 * Current creating index's name
 */
static const char *union_store_idx_name = NULL;
index_create_hook_type neon_prev_index_create = NULL;
ambuild_function neon_prev_index_build = NULL;
relation_size_hook_type neon_prev_relation_size = NULL;
is_index_access_method_hook_type neon_prev_is_index_access_method = NULL;
...
static void set_us_am_oid(void)
{
    if (!OidIsValid(US_HASH_AM_OID))
        US_HASH_AM_OID = get_index_am_oid("ushash", true);

    if (!OidIsValid(US_BTREE_AM_OID))
        US_BTREE_AM_OID = get_index_am_oid("usbtree", true);

    if (!OidIsValid(US_GIST_AM_OID))
        US_GIST_AM_OID = get_index_am_oid("usgist", true);

    if (!OidIsValid(US_GIN_AM_OID))
        US_GIN_AM_OID = get_index_am_oid("usgin", true);

    if (!OidIsValid(US_SPGIST_AM_OID))
        US_SPGIST_AM_OID = get_index_am_oid("usspgist", true);

    if (!OidIsValid(US_BRIN_AM_OID))
        US_BRIN_AM_OID = get_index_am_oid("usbrin", true);

    if (!OidIsValid(US_BITMAP_AM_OID))
        US_BITMAP_AM_OID = get_index_am_oid("usbitmap", true);

    if (!OidIsValid(UnionStoreAmId))
        UnionStoreAmId = get_table_am_oid(UNION_STORE_AM, true);

}

static Oid
get_union_store_am(void)
{
    if (OidIsValid(UnionStoreAmId))
        return UnionStoreAmId;
...
is_master_process()
{
    if (MyAuxProcType == NotAnAuxProcess && !IsBackgroundWorker)
    {
        return true;
    }
    return false;
...
static inline bool
is_unionstore_index_oid(Oid relam)
{
	if (!OidIsValid(relam)) return false;
	set_us_am_oid();
	if (relam == US_HASH_AM_OID || relam == US_BTREE_AM_OID || relam == US_GIST_AM_OID ||
		relam == US_GIN_AM_OID || relam == US_SPGIST_AM_OID || relam == US_BRIN_AM_OID ||
		relam == US_BITMAP_AM_OID)
	{
		return true;
	}
	return false;
...
static inline bool
is_unionstore_index(const char* type)
{
	if (strcmp(type, "usbtree") == 0 ||
		strcmp(type, "ushash") == 0 ||
		strcmp(type, "usgist") == 0 ||
		strcmp(type, "usgin") == 0 ||
		strcmp(type, "usspgist") == 0 ||
		strcmp(type, "usbrin") == 0 ||
		strcmp(type, "usbitmap") == 0)
	{
		return true;
	}
	return false;
...
static inline bool
RelationIsUnionStore(Relation rel)
{
	Oid relam = rel->rd_rel->relam;
    if (!OidIsValid(relam)) return false;
    set_us_am_oid();
	if (rel->rd_rel->relkind == RELKIND_INDEX)
	{
        if (is_unionstore_index_oid(relam))
        {
            return true;
        }
        return false;
...
prefetch_do_request(PrefetchRequest *slot, bool *force_latest, XLogRecPtr *force_lsn)
{
	...
	NeonGetPageRequest request = {
		...
		.local_lsn = GetUnionStoreLocalFlushLsn(),
	};
...
nm_pack_request(NeonRequest * msg)
{
	...
	switch (messageTag(msg))
	{
        ...
        case T_NeonTxnIdRequest:
        {
            NeonTxnIdRequest *msg_req = (NeonTxnIdRequest *) msg;
...
nm_unpack_response(StringInfo s)
{
	...
	switch (tag)
	{
        ...
        case T_NeonTxnIdResponse:
        {
            NeonTxnIdResponse *msg_resp = palloc0(sizeof(NeonTxnIdResponse));

            msg_resp->tag = tag;
            msg_resp->xid = pq_getmsgint64(s);
            pq_getmsgend(s);
...
nm_to_string(NeonMessage * msg)
{
	...
	switch (messageTag(msg))
	{
        ...
        case T_NeonTxnIdResponse:
            {
                NeonTxnIdResponse *msg_resp = (NeonTxnIdResponse *) msg;

                appendStringInfoString(&s, "{\"type\": \"NeonTxnIdResponse\"");
                appendStringInfo(&s, ", \"xid\": %lu}",
                                 msg_resp->xid);
                appendStringInfoChar(&s, '}');
...
{
	XLogRecPtr	lsn = PageXLogRecPtrGet(((PageHeader) (buffer))->pd_lsn);
...
neon_get_request_lsn(bool *latest, RelFileNode rnode, ForkNumber forknum, BlockNumber blkno)
{
	...
	else
	{
		...
		*latest = true;
...
#else
		flushlsn = GetUnionStoreLocalFlushLsn();
#endif
		if (lsn > flushlsn)
		{
		...
		/*
		 * use current remote flushed lsn as read lsn, this can make sure
		 * the request page is the latest version.
		 */
		lsn = GetUnionStoreRemoteFlushLsn();
	}
...
FullTransactionId
neon_get_latest_transaction_id(XLogRecPtr lsn)
{
    NeonResponse *resp;
    FullTransactionId xid;

    NeonTxnIdRequest request = {
            .req.tag = T_NeonTxnIdRequest,
            .req.latest = true,
            .req.lsn = lsn,
    };
    ...
    {
        case T_NeonTxnIdResponse:
            xid.value = ((NeonTxnIdResponse *) resp)->xid;
            break;

        case T_NeonErrorResponse:
            ereport(ERROR,
                    (errcode(ERRCODE_INTERNAL_ERROR),
                    errmsg("could not get latest transaction id from page server"),
                   errdetail("page server returned error: %s",
                 ((NeonErrorResponse *) resp)->message)));
            break;
			...
			ereport(ERROR,
					(errcode(ERRCODE_DISK_FULL),
					 errmsg("could not extend file because cluster size limit (%d MB) has been exceeded",
							max_cluster_size),
							errhint("This limit is defined by unionstore.max_cluster_size GUC")));
	}
		  ...
		  char *buffer)
{
	...
	/*
	 * The LSN on page is the WAL Service's LSN, we should set local LSN to the page.
	 * PageSetLSN(buffer, GetUnionStoreLocalFlushLsn());
	 * Local Lsn is sent to page server with page read request, page server will set the lsn.
	 */
 ...
 */
static void PG_USED_FOR_ASSERTS_ONLY
 ...
 */
static void PG_USED_FOR_ASSERTS_ONLY
neon_end_unlogged_build(SMgrRelation reln, BlockNumber n_blocks)
{
    if (!SmgrIsUnionStore(reln))
        return;

    SetLastWrittenLSNForBlockRange(UnionStoreXactLastRecEnd, reln->smgr_rnode.node, MAIN_FORKNUM, 0,
                                   n_blocks);
    SetLastWrittenLSNForRelation(UnionStoreXactLastRecEnd, reln->smgr_rnode.node, MAIN_FORKNUM);

    if (unlogged_build_rel == NULL && unlogged_build_phase == UNLOGGED_BUILD_NOT_IN_PROGRESS)
        return;
...
void
neon_smgr_init(SMgrRelation reln, BackendId backend, SMgrImpl which, Relation rel)
{
	/*
	 * We should not access Union Store in recovery mode.
	 */
	if (RecoveryInProgress())
		return;

	/*
	 * Fastpath: catalog is not Union Store table.
	 *
	 * relNode is not pg_class.oid, but less than FirstNormalObjectId must be catalog.
	 */
	if ((RelationIsValid(rel) && IsCatalogRelation(rel)) || reln->smgr_rnode.node.spcNode == GLOBALTABLESPACE_OID || reln->smgr_rnode.node.relNode < FirstNormalObjectId)
		return;

	/*
	 * set union store storage manager here
	 */
	if (which == SMGR_MD && backend == InvalidBackendId)
	{
	    if (IsUnderPostmaster && IsPostmasterEnvironment)
		{
	        if (RelationIsValid(rel))
	        {
                bool is_union_store = false;
                if (rel->rd_rel->relkind == RELKIND_INDEX)
                {
                    if (union_store_idx_name &&
                        strcmp(NameStr(rel->rd_rel->relname), union_store_idx_name) == 0)
                    {
                        /*
                         * This index is creating in progress, and is UnionStore's index.
                         */
                        is_union_store = true;
                    }
                }
                ...
                {
                    if (RelationIsUnionStore(rel)) {
                        is_union_store = true;
                    }
                }
                else
                {
                    char smgr_relpersistence = reln->smgr_relpersistence;
                    reln->smgr_relpersistence = RELPERSISTENCE_PERMANENT;
                    if (neon_exists(reln, MAIN_FORKNUM))
                    {
                        is_union_store = true;
                    }
                    else
                    {
                        reln->smgr_relpersistence = smgr_relpersistence;
                    }
                }
	...
	if (neon_prev_smgr_hook)
	    (*neon_prev_smgr_hook)(reln, backend, which, rel);
}

void
neon_ProcessUtility(PlannedStmt *pstmt, const char *queryString,
                    bool readOnlyTree,
                    ...
                    DestReceiver *dest, QueryCompletion *qc)
{
    switch (nodeTag(pstmt->utilityStmt))
    {
        case T_CreateStmt:
        {
            ...
            /*
             * Since the union store uses WAL to construct the pages of table, but temporary/unlogged table
             * does not have WAL, so temporary/unlogged table should not be used with union store.
             *
             * UnionStore does not support tablespace, set/alter tablespace is currently not supported.
             */
            if ((stmt->relKind == 'r' || stmt->relKind == 'p') &&
                ((stmt->accessMethod != NULL && strcmp(stmt->accessMethod, UNION_STORE_AM) == 0) ||
                (stmt->accessMethod == NULL && strcmp(default_table_access_method, UNION_STORE_AM) == 0)))
            {
                if (stmt->relation->relpersistence != RELPERSISTENCE_PERMANENT)
                {
                	/*
                	 * set temp/unlogged table to heap, except for partitioned table.
                	 */
                	if (stmt->accessMethod == NULL && stmt->partspec == NULL)
                	{
                		stmt->accessMethod = pstrdup("heap");
                	}
                	else
	                    ereport(ERROR,
	                            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
	                            errmsg("cannot create temporary/unlogged table with union store")));
                }
                else if (Gp_role == GP_ROLE_UTILITY)
                {
                    ereport(ERROR,
                            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                            errmsg("cannot create table with union store in utility mode")));
                }
                else if (stmt->tablespacename && get_tablespace_oid(stmt->tablespacename, false) != DEFAULTTABLESPACE_OID)
                {
                	ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
							errmsg("cannot SET TABLESPACE for union store table")));
                }
                else if (stmt->partbound)
                {
                	RangeVar   *rv = (RangeVar *) linitial(stmt->inhRelations);
                	Oid	parentOid = RangeVarGetRelid(rv, NoLock, false);
                	Oid tablespace_id = get_rel_tablespace(parentOid);

                	if (!OidIsValid(tablespace_id))
                		tablespace_id = MyDatabaseTableSpace;

                	if (tablespace_id != DEFAULTTABLESPACE_OID)
                	{
                		ereport(ERROR,
								(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
								errmsg("cannot use other tablespace for union store child table"),
								errhint("union store table should only use default tablespace")));
                	}
                }
                else if (stmt->partbound == NULL)
                {
                	Oid tablespace_id = GetDefaultTablespace(stmt->relation->relpersistence, false);
                	...
                	if (tablespace_id != DEFAULTTABLESPACE_OID)
                	{
                		ereport(ERROR,
								(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
								errmsg("cannot use other tablespace for union store table"),
								errhint("union store table should only use default tablespace")));
                	}
                }
            }
    	...
    	case T_AlterTableStmt:
    	{
    		...
    		bool is_union_store = RelationIsUnionStore(rel);
    		...
    		if (is_union_store)
    		{
    			foreach(cell, atstmt->cmds)
    			{
    				...
    				if (cmd->subtype == AT_SetUnLogged)
    				{
    					ereport(ERROR,
								(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
								errmsg("cannot set union store table to UNLOGGED")));
    				}
    				else if (cmd->subtype == AT_SetTableSpace)
    				{
    					ereport(ERROR,
								(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
								errmsg("cannot ALTER TABLESPACE for union store table")));
    				}
    			}
    		}
    	...
    	{
    		ListCell   *option;
    		AlterDatabaseStmt *stmt = (AlterDatabaseStmt *) pstmt->utilityStmt;
    		...
    		foreach(option, stmt->options)
    		{
    			...
    			if (strcmp(defel->defname, "tablespace") == 0)
    			{
                    ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
							errmsg("cannot SET TABLESPACE for database")));
    			}
    		}
    		break;
    	}
    	case T_CreateTableSpaceStmt:
    	{
    		ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					errmsg("tablespaces are not supported on union store")));
    		break;
    	}
		case T_IndexStmt:
		{
			...
			if ((stmt->accessMethod != NULL && is_unionstore_index(stmt->accessMethod)) ||
				(stmt->accessMethod == NULL && is_unionstore_index(default_index_access_method)))
			{
				if (stmt->relation->relpersistence != RELPERSISTENCE_PERMANENT)
				{
					/*
					 * create index on temp/unlogged table
					 */
					ereport(ERROR,
					        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						        errmsg(
							        "cannot create index on temporary/unlogged table with union store")));
				}
				else if (Gp_role == GP_ROLE_UTILITY)
				{
					ereport(ERROR,
					        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						        errmsg("cannot create index with union store in utility mode")));
				}
				else if (stmt->tableSpace && get_tablespace_oid(stmt->tableSpace, false) != DEFAULTTABLESPACE_OID)
				{
					ereport(ERROR,
					        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						        errmsg("cannot SET TABLESPACE for union store index")));
				}
    ...
    if (neon_prev_ProcessUtility)
        (* neon_prev_ProcessUtility) (pstmt, queryString, readOnlyTree,
...
Oid
neon_index_create(Relation heapRelation,
                  ...
                  Oid *constraintId)
{
    ...
    PG_TRY();
    {
        if (is_unionstore_index_oid(indexInfo->ii_Am))
        {
        	Oid tablespace_id = tableSpaceId ? tableSpaceId : MyDatabaseTableSpace;
        	...
        	union_store_idx_name = indexRelationName;
        }


        if (neon_prev_index_create)
        {
            idx_id = (* neon_prev_index_create)(heapRelation,
                                                indexRelationName,
                                                indexRelationId,
                                                parentIndexRelid,
                                                parentConstraintId,
        ...
        {
            idx_id = index_create_internal(heapRelation,
                                          indexRelationName,
                                          indexRelationId,
                                          parentIndexRelid,
                                          parentConstraintId,
 ...
 */
IndexBuildResult *
neon_index_build(Relation heap, Relation index, IndexInfo *indexInfo)
{
    bool reset = false;
    ...
    /*
     * Currently, Gin/Gist/SPGist are built in two phases.
     */
    if (index->rd_rel->relam == US_GIN_AM_OID || index->rd_rel->relam == US_GIST_AM_OID || index->rd_rel->relam == US_SPGIST_AM_OID) {
        index->rd_smgr->smgr_relpersistence = RELPERSISTENCE_UNLOGGED;
        mdcreate(index->rd_smgr, MAIN_FORKNUM, false);
        reset = true;
    }

    if (neon_prev_index_build)
        stats = neon_prev_index_build(heap, index, indexInfo);
    else
        stats = index->rd_indam->ambuild(heap, index, indexInfo);

    if (reset)
    {
        RelFileNodeBackend rnode = index->rd_smgr->smgr_rnode;

        Assert(index->rd_smgr->smgr_relpersistence == RELPERSISTENCE_UNLOGGED);
        index->rd_smgr->smgr_relpersistence = RELPERSISTENCE_PERMANENT;


        SetLastWrittenLSNForBlockRange(UnionStoreXactLastRecEnd, index->rd_smgr->smgr_rnode.node, MAIN_FORKNUM, 0,
                                       RelationGetNumberOfBlocks(index));
        SetLastWrittenLSNForRelation(UnionStoreXactLastRecEnd, index->rd_smgr->smgr_rnode.node, MAIN_FORKNUM);
...
/*
 * Calculate relation size of neon
 */
int64
neon_relation_size(Relation rel, ForkNumber forknum)
{
    int64		totalsize = 0;
    char	   *relationpath;
    ...
    if (RelationIsUnionStore(rel))
    {
        ...
        {
            BlockNumber n = smgrnblocks(rel->rd_smgr, forknum);
    ...
    if (neon_prev_relation_size)
        return (*neon_prev_relation_size) (rel, forknum);
...
bool
neon_is_index_access_method(Oid oid, Oid expected_oid)
{
	if (!OidIsValid(oid)) return false;
	set_us_am_oid();
	switch (expected_oid)
	{
		case BTREE_AM_OID:
			if (oid == BTREE_AM_OID || oid == US_BTREE_AM_OID)
				return true;
			break;
		case HASH_AM_OID:
			if (oid == HASH_AM_OID || oid == US_HASH_AM_OID)
				return true;
			break;
		case GIST_AM_OID:
			if (oid == GIST_AM_OID || oid == US_GIST_AM_OID)
				return true;
			break;
		case GIN_AM_OID:
			if (oid == GIN_AM_OID || oid == US_GIN_AM_OID)
				return true;
			break;
		case SPGIST_AM_OID:
			if (oid == SPGIST_AM_OID || oid == US_SPGIST_AM_OID)
				return true;
			break;
		case BRIN_AM_OID:
			if (oid == BRIN_AM_OID || oid == US_BRIN_AM_OID)
				return true;
			break;
		case BITMAP_AM_OID:
			if (oid == BITMAP_AM_OID || oid == US_BITMAP_AM_OID)
				return true;
			break;
		default:
			break;
