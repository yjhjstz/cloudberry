 ...
 *
 * IDENTIFICATION
 *    contrib/udp2/ic_common/ic_faultinjection.h
 *
 *-------------------------------------------------------------------------
 */
...
set_test_mode()
{
	if (session_param.gp_udpic_dropseg != UNDEF_SEGMENT
			|| session_param.gp_udpic_dropacks_percent != 0
			|| session_param.gp_udpic_dropxmit_percent != 0
			|| session_param.gp_udpic_fault_inject_percent != 0)
		udp_testmode = true;
	else
		udp_testmode = false;
}
...
{
	if (udp_testmode &&
		(session_param.gp_udpic_dropseg == UNDEF_SEGMENT || session_param.gp_udpic_dropseg == global_param.segindex))
	{
...
#define FINC_HAS_FAULT(type) (session_param.gp_udpic_fault_inject_bitmap & (1U << (type)))
			   ...
			   socklen_t dest_len)
{
	...
	size_t	icpkthdr_size = sizeof(icpkthdr);
	bool	is_pkt = false;
	...
	if (!testmode_inject_fault(session_param.gp_udpic_fault_inject_percent))
	...
	switch (fault_type)
	{
		case FINC_PKT_HEADER_SHORTEN:
			...
			LOG(INFO, "inject fault to sendto: FINC_PKT_HEADER_SHORTEN");
			break;
		...
		case FINC_PKT_PKT_SHORTEN:
			...
			if (length > icpkthdr_size)
				testmode_length--;
			LOG(INFO, "inject fault to sendto: FINC_PKT_PKT_SHORTEN");
			break;
		...
		case FINC_PKT_CRC_CORRUPT:
			...
			if (!session_param.gp_interconnect_full_crc)
				break;
			msg->crc++;
			LOG(INFO, "inject fault to sendto: FINC_PKT_CRC_CORRUPT");
			break;
		...
		case FINC_PKT_HEADER_LEN_ZERO:
			...
			FAULT_INJECT_BACKUP_PKT();
			msg->len = 0;
			LOG(INFO, "inject fault to sendto: FINC_PKT_HEADER_LEN_ZERO");
			break;
		...
		case FINC_PKT_HEADER_LEN_NEGATIVE:
			...
			FAULT_INJECT_BACKUP_PKT();
			msg->len = -1;
			LOG(INFO, "inject fault to sendto: FINC_PKT_HEADER_LEN_NEGATIVE");
			break;
		...
		case FINC_PKT_MISMATCH:
			...
			msg->icId = 0;
			msg->seq = 1;
			LOG(INFO, "inject fault to sendto: FINC_PKT_MISMATCH");
			break;

		case FINC_OS_EAGAIN:
			if (!FINC_HAS_FAULT(fault_type))
				break;
			LOG(INFO, "inject fault to sendto: FINC_OS_EAGAIN");
			errno = EAGAIN;
			return -1;
		...
		case FINC_OS_EINTR:
			if (!FINC_HAS_FAULT(fault_type))
				break;
			LOG(INFO, "inject fault to sendto: FINC_OS_EINTR");
			errno = EINTR;
			return -1;
		...
		case FINC_NET_PKT_DUP:
			if (!FINC_HAS_FAULT(fault_type))
				break;
			LOG(INFO, "inject fault to sendto: FINC_NET_PKT_DUP");
			if ((n = sendto(socket, buffer, testmode_length, flags, dest_addr, dest_len)) != (int)length)
				return n;
			break;
		...
		case FINC_OS_NET_INTERFACE:
			if (!FINC_HAS_FAULT(fault_type))
				break;
			LOG(INFO, "inject fault to sendto: FINC_OS_NET_INTERFACE");
			errno = EFAULT;
			return -1;
		...
		case FINC_PKT_TOO_LONG:
			if (!FINC_HAS_FAULT(fault_type) || !is_pkt)
				break;
			LOG(INFO, "inject fault to sendto: FINC_PKT_TOO_LONG");
			errno = EMSGSIZE;
			return -1;
 ...
 */
static ssize_t
testmode_recvfrom(const char *caller_name, int socket, void *buffer,
				  ...
				  socklen_t *address_len)
{
	...
	switch (fault_type)
	{
		case FINC_OS_EAGAIN:
			if (!FINC_HAS_FAULT(fault_type))
				break;
			LOG(INFO, "inject fault to recvfrom: FINC_OS_EAGAIN");
			errno = EAGAIN;
			return -1;
		...
		case FINC_OS_EINTR:
			if (!FINC_HAS_FAULT(fault_type))
				break;
			LOG(INFO, "inject fault to recvfrom: FINC_OS_EINTR");
			errno = EINTR;
			return -1;
		...
		case FINC_OS_EWOULDBLOCK:
			if (!FINC_HAS_FAULT(fault_type))
				break;
			LOG(INFO, "inject fault to recvfrom: FINC_OS_EWOULDBLOCK");
			errno = EWOULDBLOCK;
			return -1;
		...
		case FINC_NET_RECV_ZERO:
			...
			LOG(INFO, "inject fault to recvfrom: FINC_NET_RECV_ZERO");
			return 0;

		case FINC_OS_NET_INTERFACE:
			if (!FINC_HAS_FAULT(fault_type))
				break;
			LOG(INFO, "inject fault to recvfrom: FINC_OS_NET_INTERFACE");
			errno = EFAULT;
			return -1;
		...
		case FINC_NET_RECV_ERROR:
			if (!FINC_HAS_FAULT(fault_type))
				break;
			LOG(INFO, "inject fault to recvfrom: FINC_NET_RECV_ERROR");
			errno = EFAULT;
			return -1;
			  ...
			  int timeout)
{
	...
	switch (fault_type)
	{
		case FINC_OS_EINTR:
			if (!FINC_HAS_FAULT(fault_type))
				break;
			LOG(INFO, "inject fault to poll: FINC_OS_EINTR");
			errno = EINTR;
			return -1;
		...
		case FINC_OS_NET_INTERFACE:
			if (!FINC_HAS_FAULT(fault_type))
				break;
			LOG(INFO, "inject fault to poll: FINC_OS_NET_INTERFACE");
			errno = EFAULT;
			return -1;
...
testmode_socket(const char *caller_name, int domain, int type, int protocol)
{
	...
	{
		LOG(INFO, "inject fault to socket: FINC_OS_NET_INTERFACE");
		errno = ENOMEM;
		return -1;
	}
			  ...
			  const struct sockaddr *address, socklen_t address_len)
{
	if (FINC_HAS_FAULT(FINC_OS_NET_INTERFACE) &&
		testmode_inject_fault(session_param.gp_udpic_fault_inject_percent))
	{
		LOG(INFO, "inject fault to bind: FINC_OS_NET_INTERFACE");
		errno = EFAULT;
		return -1;
	}
					 ...
					 socklen_t *address_len)
{
	if (FINC_HAS_FAULT(FINC_OS_NET_INTERFACE) &&
		testmode_inject_fault(session_param.gp_udpic_fault_inject_percent))
	{
		LOG(INFO, "inject fault to getsockname: FINC_OS_NET_INTERFACE");
		errno = EFAULT;
		return -1;
	}
				   ...
				   struct addrinfo **res)
{
	if (FINC_HAS_FAULT(FINC_OS_NET_INTERFACE) &&
		testmode_inject_fault(session_param.gp_udpic_fault_inject_percent))
	{
		LOG(INFO, "inject fault to pg_getaddrinfo_all: FINC_OS_NET_INTERFACE");
		return -1;
	}
		...
		const pthread_attr_t *attr, void *(*start_routine) (void *), void *arg)
{
	if (FINC_HAS_FAULT(FINC_OS_CREATE_THREAD) &&
		testmode_inject_fault(session_param.gp_udpic_fault_inject_percent))
	{
		LOG(INFO, "inject fault to pthread_create: FINC_OS_CREATE_THREAD");
		return ENOMEM;
	}
...
#define ML_CHECK_FOR_INTERRUPTS(teardownActive) \
	testmode_check_interrupts(__FUNCTION__, teardownActive)

#define sendto(socket, buffer, length, flags, dest_addr, dest_len) \
	testmode_sendto(__FUNCTION__, socket, buffer, length, flags, dest_addr, dest_len)

#define recvfrom(socket, buffer, length, flags, address, address_len) \
	testmode_recvfrom(__FUNCTION__, socket, buffer, length, flags, address, address_len)

#define poll(fds, nfds, timeout) \
	testmode_poll(__FUNCTION__, fds, nfds, timeout)

#define socket(domain, type, protocol) \
	testmode_socket(__FUNCTION__, domain, type, protocol)

#define bind(socket, address, address_len) \
	testmode_bind(__FUNCTION__, socket, address, address_len)

#define getsockname(socket, address, address_len) \
	testmode_getsockname(__FUNCTION__, socket, address, address_len)
...
#define getaddrinfo(hostname, servname, hints, res) \
	testmode_pg_getaddrinfo_all(__FUNCTION__, hostname, servname, hints, res)

#define malloc(size) \
	testmode_malloc(__FUNCTION__, size)

#define free(ptr) \
	testmode_free(__FUNCTION__, ptr)

/*
#define ic_malloc0(size) \
	testmode_malloc0(__FUNCTION__, size)
*/

#define pthread_create(thread, attr, start_routine, arg) \
	testmode_pthread_create(__FUNCTION__, thread, attr, start_routine, arg)
#endif
