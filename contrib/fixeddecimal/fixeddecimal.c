...
fixeddecimal_numeric(PG_FUNCTION_ARGS)
{
	...
	int index = 0;
	bool neg;
	NumericDigit digits[20];
	NumericDigit *short_digits;


	if ((neg = num < 0)) num = -num;
	if (num == 0) goto quick_out;

	if (unlikely(num < 0)) {
		/*
		 * INT64_MIN: -9,223,372,036,854,775,808
		 * i.e. -9,2233,7203,6854,7758.0800
		 */
		digits[0] =  800;
		digits[1] = 7758;
		digits[2] = 6854;
	...
	integer = num / 100;
	frac = num % 100;
	Assert(integer >= 0 && frac >= 0);

	/* add digits to the array, in reverse order */
	if (frac != 0) digits[index++] = frac * 100;
	for (int64 i = integer; i != 0; i /= NBASE) digits[index++] = i % NBASE;

	/* remove head and tail zeros */
	start_index = index - 1;
	while (digits[start_index] == 0) start_index--;
	while (digits[end_index] == 0) end_index++;

	Assert(start_index >= end_index);
	ndigits = start_index - end_index + 1;
	weight = index - 1 - (int)(frac != 0);
	Assert(ndigits > 0);

quick_out:
	len = NUMERIC_HDRSZ_SHORT + ndigits * sizeof(NumericDigit);
	val = palloc(len);
	SET_VARSIZE(val, len);

	val->choice.n_short.n_header = (neg ? (NUMERIC_SHORT | NUMERIC_SHORT_SIGN_MASK) : NUMERIC_SHORT) |
				(FIXEDDECIMAL_SCALE << NUMERIC_SHORT_DSCALE_SHIFT) |
				(weight < 0 ? NUMERIC_SHORT_WEIGHT_SIGN_MASK : 0) |
				(weight & NUMERIC_SHORT_WEIGHT_MASK);

	short_digits = NUMERIC_DIGITS(val);
	for (int i = 0; i < ndigits; i++) {
		Assert(start_index - i >= 0);
		Assert(end_index <= start_index - i);
		short_digits[i] = digits[start_index - i];
	}
	PG_RETURN_NUMERIC(val);
}
...
numeric_fixeddecimal(PG_FUNCTION_ARGS)
{
	...
	if (NUMERIC_IS_SPECIAL(num))
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("cannot convert NaN/Inf to fixeddecimal")));
	...
	if (var.ndigits == 0) return 0;

	val = 0;
	for (index = 0; index <= var.weight; index++) {
		tmp64 = val * NBASE;
		if (index < var.ndigits) tmp64 += var.digits[index];
		if (unlikely(tmp64 < val)) goto overflow;
		val = tmp64;
	}

	/* handle fraction part */
	tmp64 = val * 100;
	if (index < var.ndigits) {
		tmp64 += var.digits[index] / 100;
		if (unlikely((var.digits[index] % 100) != 0) || index + 1 != var.ndigits)
			elog(ERROR, "fixeddecimal: scale > 2");
	}

	if (unlikely(tmp64 == INT64_MIN)
		? var.sign != NUMERIC_NEG
		: unlikely(tmp64 < val))
		goto overflow;

	val = tmp64;
	result = var.sign == NUMERIC_NEG ? -val : val;
	PG_RETURN_DATUM(result);
...
fixeddecimal_avg(PG_FUNCTION_ARGS)
{
	...
	Datum sum = DirectFunctionCall1(fixeddecimal_numeric, Int64GetDatum(state->sumX));
	Datum n = NumericGetDatum(int64_to_numeric(state->N));
...
fixeddecimal_sum(PG_FUNCTION_ARGS)
{
	...
	return DirectFunctionCall1(fixeddecimal_numeric, Int64GetDatum(state->sumX));
}
