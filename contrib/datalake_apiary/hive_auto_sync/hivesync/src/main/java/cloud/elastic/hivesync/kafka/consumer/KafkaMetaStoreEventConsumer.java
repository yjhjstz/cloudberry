package cloud.elastic.hivesync.kafka.consumer;

import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.APPLICATION_NAME;
import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.BOOTSTRAP_SERVERS;
import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.HDW_DB;
import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.HIVE_CATALOG;
import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.HIVE_CATLIST;
import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.HIVE_CLUSTER_NAME;
import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.KAFKA_MECHANISM;
import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.KAFKA_SASL_JAAS_CONFIG;
import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.KAFKA_SECURITY;
import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.MAX_POLL_INTERVAL_MS;
import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.MAX_POLL_RECORDS;
import static cloud.elastic.hivesync.kafka.consumer.HiveAutoSyncProperty.TOPIC_NAME;
...
import cloud.elastic.hivesync.kafka.prometheus.HiveSyncMetrics;
...
public class KafkaMetaStoreEventConsumer implements Runnable{
    ...
    private Map<String, ElasticConnector> connectorMap = new HashMap<String, ElasticConnector>();
    ...
    private void pullFromKafka() {
        while (kafkaMessageReader.hasNext()) {
            try {
                ...
                for (ElasticConnector connector : connectorMap.values()) {
                    connector.close();
                }
    ...
    private void syncEvent(ApiaryListenerEvent event) {
        ...
        ElasticConnector ElasticConnector = connectorMap.get(catalog);
        if (ElasticConnector == null) {
            log.info("No elastic connector for catalog " + catalog);
            return;
        }
        ...
        if (et == EventType.ON_CREATE_TABLE) {
            syncResult = ElasticConnector.createTable(event.getDatabaseName(), event.getTableName(), majorVer);
        }
        else if (et == EventType.ON_ALTER_TABLE) {
            if (majorVer == 2) {
                syncResult = ElasticConnector.alterTable(event.getDatabaseName(), event.getTableName(), majorVer);
            ...
            syncResult = ElasticConnector.dropTable(event.getDatabaseName(), event.getTableName());
        }
        else if (et == EventType.ON_DROP_PARTITION || et == EventType.ON_ALTER_PARTITION) {
            syncResult = ElasticConnector.alterPartition(event.getDatabaseName(), event.getTableName());
        }
        else if (et == EventType.ON_DROP_DATABASE) {
            syncResult = ElasticConnector.dropSchema(event.getDatabaseName());
        }
    ...
    public void run() {
        ...
        for (String catalog : catalogList) {
            try {
                ...
                connectorMap.put(catalog, new ElasticConnector(props));
                HiveSyncMetrics.incrementConnectorCounts();
                log.info("Build elastic connector " + props.getProperty(HDW_DB.key()) + " success!");
            } catch (Exception e) {
                log.error("Build elastic connector " + props.getProperty(HDW_DB.key()) + " failed!", e);
            }
        }
        if (connectorMap.isEmpty()) {
            log.error("No elastic connector is built, stop syncer for cluster " + props.getProperty(HIVE_CLUSTER_NAME.key()));
            return;
        }
        buildKafkaReader();
