...
import static cn.hashdata.hivesync.kafka.consumer.HiveAutoSyncProperty.HIVE_CATALOG;
...
import static cn.hashdata.hivesync.kafka.consumer.HiveAutoSyncProperty.MAX_POLL_INTERVAL_MS;
import static cn.hashdata.hivesync.kafka.consumer.HiveAutoSyncProperty.MAX_POLL_RECORDS;
...
import cn.hashdata.hivesync.kafka.prometheus.HiveSyncMetrics;
...
public class KafkaMetaStoreEventConsumer implements Runnable{
    ...
    private Map<String, HashdataConnector> connectorMap = new HashMap<String, HashdataConnector>();
    ...
    private void pullFromKafka() {
        while (kafkaMessageReader.hasNext()) {
            try {
                ...
                for (HashdataConnector connector : connectorMap.values()) {
                    connector.close();
                }
    ...
    private void syncEvent(ApiaryListenerEvent event) {
        ...
        HashdataConnector hashdataConnector = connectorMap.get(catalog);
        if (hashdataConnector == null) {
            log.info("No hashdata connector for catalog " + catalog);
            return;
        }
        ...
        if (et == EventType.ON_CREATE_TABLE) {
            syncResult = hashdataConnector.createTable(event.getDatabaseName(), event.getTableName(), majorVer);
        }
        else if (et == EventType.ON_ALTER_TABLE) {
            if (majorVer == 2) {
                syncResult = hashdataConnector.alterTable(event.getDatabaseName(), event.getTableName(), majorVer);
            ...
            syncResult = hashdataConnector.dropTable(event.getDatabaseName(), event.getTableName());
        }
        else if (et == EventType.ON_DROP_PARTITION || et == EventType.ON_ALTER_PARTITION) {
            syncResult = hashdataConnector.alterPartition(event.getDatabaseName(), event.getTableName());
        }
        else if (et == EventType.ON_DROP_DATABASE) {
            syncResult = hashdataConnector.dropSchema(event.getDatabaseName());
        }
    ...
    public void run() {
        ...
        for (String catalog : catalogList) {
            try {
                ...
                connectorMap.put(catalog, new HashdataConnector(props));
                HiveSyncMetrics.incrementConnectorCounts();
                log.info("Build hashdata connector " + props.getProperty(HDW_DB.key()) + " success!");
            } catch (Exception e) {
                log.error("Build hashdata connector " + props.getProperty(HDW_DB.key()) + " failed!", e);
            }
        }
        if (connectorMap.isEmpty()) {
            log.error("No hashdata connector is built, stop syncer for cluster " + props.getProperty(HIVE_CLUSTER_NAME.key()));
            return;
        }
        buildKafkaReader();
