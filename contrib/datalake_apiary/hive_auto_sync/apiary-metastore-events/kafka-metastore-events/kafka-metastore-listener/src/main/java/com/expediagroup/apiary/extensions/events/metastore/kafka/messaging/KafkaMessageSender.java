...
import static com.expediagroup.apiary.extensions.events.metastore.kafka.messaging.KafkaProducerProperty.KAFKA_MECHANISM;
import static com.expediagroup.apiary.extensions.events.metastore.kafka.messaging.KafkaProducerProperty.KAFKA_SASL_JAAS_CONFIG;
import static com.expediagroup.apiary.extensions.events.metastore.kafka.messaging.KafkaProducerProperty.KAFKA_SECURITY;
...
public class KafkaMessageSender {
  ...
  public void send(KafkaMessage kafkaMessage) {
    ...
    try {
      producer.send(new ProducerRecord<>(topic, partition, kafkaMessage.getTimestamp(), kafkaMessage.getPayload())).get();
      producer.flush();
    } catch (Exception e) {
  ...
  static Properties kafkaProperties(Configuration conf) {
    ...
    String kafkaProtocol = stringProperty(conf, KAFKA_SECURITY);
    ...
    if (kafkaProtocol.equals("SASL_PLAINTEXT")) {
      ...
      props.put(KAFKA_SASL_JAAS_CONFIG.unprefixedKey(), "cloud.elastic.apiary.shaded." + stringProperty(conf, KAFKA_SASL_JAAS_CONFIG));
    }
