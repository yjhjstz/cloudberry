...
WHERE (p1.typnamespace = 0 OR
    ...
    (p1.typstorage not in ('p', 'x', 'e', 'm'))) AND
    (p1.typname != 'int128') AND
    (p1.typname != 'stddev');
...
WHERE p1.typinput = p2.oid AND p1.typtype in ('b', 'p') AND NOT
    ...
    (p2.prorettype = p1.oid AND NOT p2.proretset) AND
    (p1.typname != 'int128') AND
    (p1.typname != 'stddev')
ORDER BY 1;
    ...
    (p2.pronargs = 1 AND
     (p2.proargtypes[0] = p1.oid OR
      (p2.oid = 'array_out'::regproc AND
       p1.typelem != 0 AND p1.typlen = -1))) AND
       p1.typname != 'int128' AND
       p1.typname != 'stddev'
ORDER BY 1;
...
FROM pg_class as p1
WHERE relkind NOT IN ('r', 'i', 'S', 't', 'v', 'm', 'c', 'f', 'p', 'o', 'M','b') OR
...
-- start_matchignore
-- m/\b\d+\s*\|\s*int128\s*\|\s*\w\s*\|\s*\d+\s*\|\s*\d+\s*\|\s*\d+\b/
-- s/\b\d+\s*\|\s*int128\s*\|\s*\w\s*\|\s*\d+\s*\|\s*\d+\s*\|\s*\d+\b/
-- end_matchignore
