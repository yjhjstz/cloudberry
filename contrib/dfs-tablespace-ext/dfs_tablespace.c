	...
	foreach (option, stmt->options)
	{
		...
		if (pg_strcasecmp(defel->defname, "full") == 0)
		{
			...
			foreach (lc, stmt->rels)
			{
				...
				{
					elog(ERROR, "vacuum full is not supported for tables which in tablespaces of type dfs_tablespace.");
				}
				table_close(rel, AccessShareLock);
			}
		}
...
static void ValidateAlterTableStmt(AlterTableStmt *stmt)
{
	...
	if (IsDfsTablespaceById(rel->rd_rel->reltablespace))
	{
		elog(ERROR, "changing the dfs tablespace type in alter table stmt is not supported");
	}
	...
	foreach (lcmd, stmt->cmds)
	{
		...
		if (cmd->subtype == AT_SetTableSpace)
		{
			...
			/* alter table stmt does not support setting tablespace of dfs tablespace type */
			if (OidIsValid(tablespaceId) && IsDfsTablespaceById(tablespaceId))
			{
				elog(ERROR, "changing the dfs tablespace type in alter table stmt is not supported");
			}
		}
...
static void ValidateAlterDatabaseStmt(AlterDatabaseStmt *stmt)
{
	...
	foreach (lcmd, stmt->options)
	{
		...
		if (pg_strcasecmp(defel->defname, "tablespace") == 0)
		{
			...
			/* alter database stmt does not support setting tablespace of dfs tablespace type */
			if (OidIsValid(tablespaceId) && IsDfsTablespaceById(tablespaceId))
			{
				elog(ERROR, "changing dfs tablespace in alter database stmt is not supported");
			}
		}
	}
}

void checkUnsupportDfsTableSpaceStmt(Node *stmt)
{
	switch (nodeTag(stmt))
	{
