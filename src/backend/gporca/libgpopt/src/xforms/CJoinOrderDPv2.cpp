	...
	while (iter_pt.Advance())
	{
		...
		if (!part_table_group_info->IsAnAtom())
		{
				...
				nullptr != left_child_join_condition_cols)
			{
				...
				// Attempt to get distribution columns using safe version
				// PcrsDistSafe handles subset case - sets fComplete to false if distribution
				// columns are not fully present in output (e.g., in semi-joins, aggregations)
				BOOL fComplete = false;
				CColRefSet *pt_atom_distribution_cols = CLogical::PcrsDistSafe(
					m_mp, atom_table_descriptor, atomOutputColArray, &fComplete);
