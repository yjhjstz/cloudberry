...
CCostContext::IsValid(CMemoryPool *mp)
{
	...
	if ((COptCtxt::FAllEnforcersEnabled() && !fValid) ||
		GPOS_FTRACE(EopttracePrintPlanSelection))
	{
		CAutoTrace at(mp);
		IOstream &os = at.Os();
...
CCostContext::FBetterThan(const CCostContext *pcc) const
{
	...
	// Print cost comparison trace
	if (GPOS_FTRACE(EopttracePrintPlanSelection))
	{
		CAutoTrace at(m_mp);
		IOstream &os = at.Os();
		os << std::endl
		   << "=== COST COMPARISON ===" << std::endl
		   << "Group: " << Pgexpr()->Pgroup()->Id()
		   << ", OptCtxt: " << m_poc->Id() << std::endl
		   << "  Candidate (new): Expr " << Pgexpr()->Id()
		   << " [" << Pgexpr()->Pop()->SzId() << "]"
		   << " Cost=" << Cost() << std::endl
		   << "  Existing:        Expr " << pcc->Pgexpr()->Id()
		   << " [" << pcc->Pgexpr()->Pop()->SzId() << "]"
	...
	if (dCostDiff < 0.0)
	{
		...
		{
			CAutoTrace at(m_mp);
			at.Os() << "  Result: NEW WINS (lower cost)" << std::endl;
		}
		return true;
	}
	...
	if (dCostDiff > 0.0)
	{
		...
		{
			CAutoTrace at(m_mp);
			at.Os() << "  Result: EXISTING WINS (lower cost)" << std::endl;
		}
		return false;
	}
